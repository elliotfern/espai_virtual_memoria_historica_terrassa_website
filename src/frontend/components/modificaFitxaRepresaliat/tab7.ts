...
function crearBotoAfegirBibliografia(idPersona: number | string) {
  ...
  updateBtn.onclick = (event) => {
    ...
    // Limpiar solo la tabla de bibliografía
    const cont = document.getElementById('quadreFontsBibliografia');
...
function crearBotoAfegirArxiu(idPersona: number | string) {
  ...
  updateBtn.onclick = (event) => {
    event.preventDefault();
    // Limpiar solo la tabla de arxius
    const cont = document.getElementById('quadreFontsArxius');
    if (cont) cont.innerHTML = '';
