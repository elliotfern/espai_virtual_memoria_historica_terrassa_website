...
export function tab4(fitxa: Fitxa) {
  auxiliarSelect(fitxa.estudis_id, 'estudis', 'estudis', 'estudi_cat');
  ...
  auxiliarSelect(fitxa.sector_id, 'sectors_economics', 'sector', 'sector_cat');
  ...
  if (empresaInput && fitxa.empresa !== undefined) {
    empresaInput.value = fitxa.empresa;
  }
