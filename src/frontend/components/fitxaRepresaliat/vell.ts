...
export async function fitxaRepresaliat(slug: string) {
  // cridar fetch
  const fetchData = await fetchDataFitxaRepresaliat(slug);
  if (fetchData) {
    fetchDataFitxaRepresaliatFamiliar(fetchData.id);
    initButtons(fetchData.id);
  }
}

// Variable para almacenar la información obtenida de la API
const cachedData: Fitxa | null = null;
let cachedData2: FitxaFamiliars[];

export async function initButtons(id: number): Promise<void> {
...
async function generarBotonesCategoria(idPersona: number): Promise<void> {
...
async function mostrarCategoria(categoriaNumerica: string, idPersona: number): Promise<void> {
  const divInfo = document.getElementById('fitxa-categoria');
...
async function mostrarInformacion(tab: string, idPersona: number, label: string): Promise<void> {
