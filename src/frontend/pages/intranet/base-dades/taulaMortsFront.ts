...
export async function taulaMortsFronts() {
  const isAdmin = await getIsAdmin();
  const isAutor = await getIsAutor();
  const isLogged = await getIsLogged();
  const reloadKey = 'reload-taula-taulaLlistatMortsFront';
  ...
  const columns: Column<EspaiRow>[] = [
    ...
    { header: 'Fitxa mort al front creada', field: 'es_mortFront' },
  ];
  ...
  if (isAdmin || isAutor || isLogged) {
    columns.push({
      header: 'Accions',
      field: 'id',
      render: (_: unknown, row: EspaiRow) => `<a id="${row.id}" title="Modifica" target="_blank" href="https://${window.location.hostname}/gestio/base-dades/modifica-repressio/3/${row.id}"><button type="button" class="btn btn-warning btn-sm">Modifica repressio</button></a>`,
    });
  }
  ...
  renderTaulaCercadorFiltres<EspaiRow>({
    url: API_URLS.GET.LLISTAT_MORTS_FRONT,
    containerId: 'taulaLlistatMortsFront',
    columns,
  ...
  registerDeleteCallback(reloadKey, () => taulaMortsFronts());
