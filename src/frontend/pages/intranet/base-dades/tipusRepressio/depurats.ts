...
export async function depurats(idRepresaliat: number) {
  ...
  const response = await fetchDataGet<Fitxa>(
    `/api/depurats/get/fitxaRepressio?id=${idRepresaliat}`
  );
  ...
  if (!response) {
    if (htmlForm) {
      htmlForm.addEventListener('submit', function (event) {
        transmissioDadesDB(event, 'POST', 'depuratForm', `${ENV.apiBaseUrl}/depurats/post`, true);
      });
    }
  } else {
    if (htmlForm) {
      htmlForm.addEventListener('submit', function (event) {
        transmissioDadesDB(event, 'PUT', 'depuratForm', `${ENV.apiBaseUrl}/depurats/put`);
      });
    }
  }
}
