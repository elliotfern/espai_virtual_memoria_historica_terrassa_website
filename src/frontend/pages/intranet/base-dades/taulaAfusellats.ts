...
export async function taulaAfusellats() {
  const isAdmin = await getIsAdmin();
  const isAutor = await getIsAutor();
  const isLogged = await getIsLogged();
  const reloadKey = 'reload-taula-taulaLlistatAfusellats';
  ...
  const columns: Column<EspaiRow>[] = [
    ...
    { header: 'Fitxa afusellat creada', field: 'es_afusellat' },
  ];
  ...
  renderTaulaCercadorFiltres<EspaiRow>({
    url: API_URLS.GET.LLISTAT_AFUSELLATS,
    containerId: 'taulaLlistatAfusellats',
    columns,
  ...
  registerDeleteCallback(reloadKey, () => taulaAfusellats());
