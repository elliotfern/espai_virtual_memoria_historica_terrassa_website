...
export async function taulaPaginaCostHuma(): Promise<void> {
  const isAdmin = await getIsAdmin();
  const isAutor = await getIsAutor();
  const isLogged = await getIsLogged();
  const reloadKey = 'reload-taula-taulaCostHuma';
  ...
  const res = await fetch(API_URLS.GET.LLISTAT_COSTHUMA_INTRANET);
  ...
  registerDeleteCallback(reloadKey, () => taulaPaginaCostHuma());
  initDeleteHandlers();
}
