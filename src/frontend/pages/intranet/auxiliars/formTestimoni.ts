...
export async function formTestimoni(isUpdate: boolean, id?: number) {
  ...
  if (id && isUpdate) {
    const response = await fetchDataGet<ApiResponse<Fitxa>>(`https://memoriaterrassa.cat/api/processats/get/testimoniAcusacio?id=${id}`);
