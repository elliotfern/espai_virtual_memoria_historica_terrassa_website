...
export async function llistatTribunalVocals() {
  ...
  if (isAdmin || isAutor) {
    columns.push({
      ...
      render: (_: unknown, row: EspaiRow) =>
        `<a id="${row.id}" title="Modifica" href="${ENV.domainWeb}/gestio/auxiliars/modifica-vocal-tribunal/${row.id}"><button type="button" class="btn btn-warning btn-sm">Modifica</button></a>`,
    });
  }
  ...
  renderTaulaCercadorFiltres<EspaiRow>({
    url: `${ENV.apiBaseUrl}/auxiliars/get/tribunals_vocals`,
    containerId: 'tabla1',
    columns,
    //filterKeys: ,
