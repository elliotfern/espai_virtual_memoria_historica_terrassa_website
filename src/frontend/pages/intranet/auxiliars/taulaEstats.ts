...
export async function taulaEstats() {
  const isAdmin = await getIsAdmin();
  const isAutor = await getIsAutor();

  const columns: Column<EspaiRow>[] = [{ header: 'Estat', field: 'estat' }];
  ...
  renderTaulaCercadorFiltres<EspaiRow>({
    url: `https://${window.location.host}/api/auxiliars/get/estats`,
    containerId: 'taulaLlistatEstats',
    columns,
    filterKeys: ['comarca'],
  ...
  initDeleteHandlers(() => taulaEstats()); // Recargar tabla después de eliminar
}
