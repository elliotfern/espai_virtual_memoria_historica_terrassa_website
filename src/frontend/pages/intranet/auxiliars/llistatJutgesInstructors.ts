...
export async function llistatJutgesInstructors() {
  ...
  if (isAdmin || isAutor) {
    columns.push({
      ...
      render: (_: unknown, row: EspaiRow) =>
        `<a id="${row.id}" title="Modifica" href="${ENV.domainWeb}/gestio/auxiliars/modifica-jutge-instructor/${row.id}"><button type="button" class="btn btn-warning btn-sm">Modifica</button></a>`,
    });
  }
  ...
  renderTaulaCercadorFiltres<EspaiRow>({
    url: `${ENV.apiBaseUrl}/auxiliars/get/jutges_instructors`,
    containerId: 'tabla1',
    columns,
    //filterKeys: ,
