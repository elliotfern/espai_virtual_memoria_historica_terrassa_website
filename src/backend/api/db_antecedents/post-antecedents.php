...
/**
 * POST : Crear antecedent
 * URL: /api/antecedents/post/antecedent
 */
if ($slug === "antecedent") {
    ...
    // Validar image_id si ve informat
    ...
    try {
        ...
        // Validacions de FK abans de transacció
        if ($imageId !== null) {
            // Ajusta el nom de la taula si la teva taula real d'imatges és una altra
            ...
            if (!$stmtImage->fetchColumn()) {
                Response::error("La imatge indicada no existeix.", [], 400);
                return;
            }
        }
        ...
        // 1) INSERT principal
        $sqlAntecedent = "
            INSERT INTO db_web_antecedents
                (ordre, image_id, layout_image_left, show_in_timeline, date_created)
            VALUES
                (:ordre, :image_id, :layout_image_left, :show_in_timeline, NOW())
        ";
        ...
        $antecedentId = (int)$conn->lastInsertId();
        ...
        foreach ($i18nRows as $lang => $row) {
            ...
            $stmtI18n->bindValue(':antecedent_id', $antecedentId, PDO::PARAM_INT);
        ...
        Response::success(
            "L'antecedent s'ha desat correctament.",
            ['id' => $antecedentId],
            200
        );
        return;
