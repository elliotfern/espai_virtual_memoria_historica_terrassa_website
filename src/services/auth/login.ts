...
export function loginPage() {
  ...
  const loginMessageErr = document.querySelector(
    "#loginMessageErr"
  ) as HTMLElement;
  ...
  btnLogin?.addEventListener("click", (event: Event) => {
    ...
    if (errors.length > 0) {
      loginMessageErr.innerHTML = `<ul class="mb-0">${errors
        .map((err) => `<li>${err}</li>`)
        .join("")}</ul>`;
...
export async function login(email: string, password: string): Promise<void> {
  const urlAjax = `https://api.memoriaterrassa.cat/login`;
  ...
  const loginMessageOk = document.querySelector(
    "#loginMessageOk"
  ) as HTMLElement;
  const loginMessageErr = document.querySelector(
    "#loginMessageErr"
  ...
  try {
    ...
    if (response.ok && data.status === "success") {
      // ✅ Login exitoso
      loginMessageOk.innerHTML =
        data.message || "Has iniciat sessió correctament. Redirigint...";
      loginMessageOk.style.display = "block";

      setTimeout(() => {
        loginMessageOk.style.display = "none"; // Ocultar el mensaje de éxito
        window.location.href = `/inici`; // Redirigir a la página de inicio
      }, 1300); // Muestra el mensaje durante 1.3 segundos antes de redirigir
    } else {
      ...
      const message =
        data?.status === "error"
          ...
          : "Error desconegut en iniciar sessió.";
