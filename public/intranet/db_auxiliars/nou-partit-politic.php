...
<div class="container" style="margin-bottom:50px;border: 1px solid gray;border-radius: 10px;padding:25px;background-color:#eaeaea">
    <form id="causaMortForm">
        <div class="container">
            <div class="row">
                <h2>Crear nou partit polític</h2>
...
<script>
    ...
    async function enviarFormulario(event) {
        ...
        const devDirectory = `https://${window.location.hostname}`;
        let urlAjax = devDirectory + "/api/auxiliars/put/type=partit_politic";
    ...
    async function enviarFormularioPost(event) {
        ...
        const jsonData = JSON.stringify(formData);
        const devDirectory = `https://${window.location.hostname}`;
        let urlAjax = devDirectory + "/api/auxiliars/post/?type=partit_politic";
