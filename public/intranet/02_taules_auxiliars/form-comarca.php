    ...
    <form id="form">
        <div class="container">
            <?php if ($modificaBtn === 1) { ?>
                ...
                <h4 id="fitxaMunicipi">Comarca: <?php echo $comarca_old; ?></h4>
            <?php } else { ?>
...
<script>
    ...
    async function enviarFormulario(event) {
        ...
        let urlAjax = devDirectory + "/api/auxiliars/put/comarca";
    ...
    async function enviarFormularioPost(event) {
        ...
        const jsonData = JSON.stringify(formData);
        const devDirectory = `https://${window.location.hostname}`;
        let urlAjax = devDirectory + "/api/auxiliars/post/comarca";
        ...
        try {
            ...
            if (data.status === "success") {
                ...
                if (okMessageDiv && okTextDiv && errMessageDiv) {
                    okMessageDiv.style.display = "block";
                    okTextDiv.innerHTML = data.message || "Les dades s'han desat correctament!";
